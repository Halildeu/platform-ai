...
| **speechbrain ECAPA** | Apache-2.0 | **no** (free) | 56.64%¹ | **307 MB**² | **0.006** | 7× less GPU0-total², 9× faster |
...
> `diar_matrix.py` also emits `peak_vram_delta_mb` (peak − pre-load baseline) and
> the duration-weighted `der_corpus` (the real decision metric); both land in the
